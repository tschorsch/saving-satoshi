const translations = {
  ...
  chapter_eight: {
    ...
    building_blocks_five: {
      ...
      paragraph_two: `"inputs" 配列はそのトランザクションで使用された（破棄された）コインで、"outputs" 配列は新しく作られたコインです。トランザクション手数料は、マイナーにそのトランザクションをブロックへ取り込む動機を与えます。手数料は、入力値の合計から出力値の合計を引いた差額です。`,
...
スタックのヒント：スクリプトを満たすには、2つの署名を提供する必要があります。スタック上の0は、CHECKMULTISIG の仕様に伴う特別な動作に対応するためのものです。`,
      ...
      proposal_three: {
        ...
        spoiler: `スクリプトのヒント：このスクリプトは2つの条件で送金を許可します：ブロック高6930300より前か後かです。ブロック前はあなたが送金でき、ブロック後はヴァンダープールが送金できます。

スタックのヒント：指定されたブロックの前に送金するには、あなたの署名を使います。ブロック後であれば、ヴァンダープールの署名と 0 を提供します。これはスクリプトがタイムロックの検証を終えた後の状態を示します。`,
      },
      proposal_four: {
